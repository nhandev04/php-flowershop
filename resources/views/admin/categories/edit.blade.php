...
@section('content')
    <div class="container mx-auto px-4">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white transition-colors duration-300">Chỉnh sửa danh mục</h1>
            ...
            <form action="{{ route('admin.categories.update', $category->id) }}" method="POST"
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="shadow appearance-none border border-gray-300 dark:border-gray-600 rounded w-full py-2 px-3 bg-white dark:bg-gray-700 text-gray-700 dark:text-white leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-300 @error('description') border-red-500 dark:border-red-400 @enderror">{{ old('description', $category->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 dark:text-red-400 text-xs italic mt-1 transition-colors duration-300">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 text-xs mt-1 transition-colors duration-300">Để trống nếu không muốn thay đổi hình ảnh hiện tại</p>
                    @error('image')
                        <p class="text-red-500 dark:text-red-400 text-xs italic mt-1 transition-colors duration-300">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }} class="mr-2 h-4 w-4 text-blue-600 border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 rounded focus:ring-blue-500 dark:focus:ring-blue-400 transition-colors duration-300">
                        <label for="is_active" class="text-gray-700 dark:text-gray-300 text-sm font-bold transition-colors duration-300">Hiển thị</label>
                    </div>
                    @error('is_active')
                        <p class="text-red-500 dark:text-red-400 text-xs italic mt-1 transition-colors duration-300">{{ $message }}</p>
                    @enderror
