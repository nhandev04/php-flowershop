...
@section('content')
    <div class="container mx-auto px-4">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white transition-colors duration-300">Chi tiết danh mục</h1>
            <div>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="mb-4">
                            <p class="text-gray-600 dark:text-gray-400 text-sm transition-colors duration-300">Mã danh mục</p>
                            <p class="font-medium text-gray-900 dark:text-white transition-colors duration-300">{{ $category->id }}</p>
                        </div>

                        <div class="mb-4">
                            <p class="text-gray-600 dark:text-gray-400 text-sm transition-colors duration-300">Tên danh mục</p>
                            <p class="font-medium text-gray-900 dark:text-white transition-colors duration-300">{{ $category->name }}</p>
                        </div>

                        <div class="mb-4">
                            <p class="text-gray-600 dark:text-gray-400 text-sm transition-colors duration-300">Mô tả</p>
                            <p class="font-medium text-gray-900 dark:text-white transition-colors duration-300">{{ $category->description ?: 'Chưa có mô tả' }}</p>
                        </div>

                        <div class="mb-4">
                            <p class="text-gray-600 dark:text-gray-400 text-sm transition-colors duration-300">Thứ tự hiển thị</p>
                            <p class="font-medium text-gray-900 dark:text-white transition-colors duration-300">{{ $category->sort_order }}</p>
                        </div>

                        <div class="mb-4">
                            <p class="text-gray-600 dark:text-gray-400 text-sm transition-colors duration-300">Trạng thái</p>
                            @if($category->is_active)
                                <span class="bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-200 py-1 px-3 rounded-full text-xs transition-colors duration-300">Đang hoạt động</span>
                            @else
                                <span class="bg-red-100 dark:bg-red-900/20 text-red-800 dark:text-red-200 py-1 px-3 rounded-full text-xs transition-colors duration-300">Ngưng hoạt động</span>
                            @endif
                        </div>

                        <div class="mb-4">
                            <p class="text-gray-600 dark:text-gray-400 text-sm transition-colors duration-300">Ngày tạo</p>
                            <p class="font-medium text-gray-900 dark:text-white transition-colors duration-300">{{ $category->created_at->format('d/m/Y H:i') }}</p>
                        </div>

                        <div class="mb-4">
                            <p class="text-gray-600 dark:text-gray-400 text-sm transition-colors duration-300">Cập nhật lần cuối</p>
                            <p class="font-medium text-gray-900 dark:text-white transition-colors duration-300">{{ $category->updated_at->format('d/m/Y H:i') }}</p>
                        </div>
                    </div>
                ...
                <div class="mt-8">
                    <h3 class="text-xl font-bold mb-4 text-gray-900 dark:text-white transition-colors duration-300">Danh sách sản phẩm trong danh mục</h3>
                                        ...
                                        <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-300">
                                            ...
                                            <td class="py-3 px-6 text-center">
                                                @if($product->is_active)
                                                    <span class="bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-200 py-1 px-3 rounded-full text-xs transition-colors duration-300">Đang bán</span>
                                                @else
                                                    <span class="bg-red-100 dark:bg-red-900/20 text-red-800 dark:text-red-200 py-1 px-3 rounded-full text-xs transition-colors duration-300">Ngưng bán</span>
                                                @endif
                                            </td>
                                            <td class="py-3 px-6 text-center">
                                                <div class="flex item-center justify-center">
                                                    <a href="{{ route('admin.products.show', $product->id) }}"
                                                        class="w-4 mr-2 transform hover:text-blue-500 dark:hover:text-blue-400 hover:scale-110 transition-colors duration-300" title="Xem chi tiết">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                    <a href="{{ route('admin.products.edit', $product->id) }}"
                                                        class="w-4 mr-2 transform hover:text-yellow-500 dark:hover:text-yellow-400 hover:scale-110 transition-colors duration-300" title="Chỉnh sửa">
                    ...
                    @else
                        <p class="text-gray-500 dark:text-gray-400 transition-colors duration-300">Chưa có sản phẩm nào trong danh mục này.</p>
                    @endif
                </div>
            </div>
        </div>
