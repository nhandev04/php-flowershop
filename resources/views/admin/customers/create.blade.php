...
@section('content')
    <div class="container mx-auto px-4">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white transition-colors duration-300">Thêm khách hàng mới</h1>
            ...
            <form action="{{ route('admin.customers.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="address" id="address" rows="3"
                        class="shadow appearance-none border border-gray-300 dark:border-gray-600 rounded w-full py-2 px-3 bg-white dark:bg-gray-700 text-gray-700 dark:text-white leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-300 @error('address') border-red-500 dark:border-red-400 @enderror">{{ old('address') }}</textarea>
                    @error('address')
                        <p class="text-red-500 dark:text-red-400 text-xs italic mt-1 transition-colors duration-300">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    <label for="password" class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2 transition-colors duration-300">Mật khẩu *</label>
                ...
                <div class="flex items-center justify-between">
                    <button type="submit"
                        ...
                        <i class="fas fa-save mr-2"></i> Thêm khách hàng
                    </button>
                </div>
            </form>
        </div>
